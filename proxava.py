...
def __init__(self, config):
    ...
    self.serverSocket.bind(config['HOST_NAME'], config['BIND_PORT']) # bind socket to host and port
...
request = conn.recv(config['max req len']) # get request
fline = request.split('\n')[0] # parse 1st line
...
s.settimeout(config['conn time out'])
s.connect((webserver, port))
s.sendall(request)
# receive
while 1: # True
    data = s.recv(config['max req len'])
    if (len(data)>0):
